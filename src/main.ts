...
import { BusinessesController, requireSaasAuth } from './modules/businesses/businesses.controller';
...
app.post('/api/chat', chatLimiter, ChatController.handle);
...
// Multi-tenant agent management — consumed by the SaaS panel (BarberiaAppv)
app.get('/api/agents',                   requireSaasAuth, BusinessesController.list);
app.patch('/api/agents/:id',             requireSaasAuth, BusinessesController.update);
app.post('/api/agents/:id/toggle',       requireSaasAuth, BusinessesController.toggle);
app.post('/api/agents/:id/test-wa',      requireSaasAuth, BusinessesController.testWa);
app.get('/api/agents/:id/metrics',       requireSaasAuth, BusinessesController.metrics);
...
async function autoConfigureNgrokWebhook() {
  ...
  logger.error('ngrok no respondió en 30s, abortando ejecución forzosamente');
  process.exit(1);
}
