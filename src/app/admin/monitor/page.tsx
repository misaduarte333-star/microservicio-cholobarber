...
import { Activity, MessageSquare, Clock, Zap, Cpu, AlertCircle, Terminal, Bot, Code } from 'lucide-react'
...
export default function AIMonitorPage() {
    ...
    const [promptData, setPromptData] = useState<{prompt: string, lastUpdated: string} | null>(null)
    ...
    const fetchData = useCallback(async () => {
        ...
        try {
            ...
            if (!promptJson.error) setPromptData(promptJson)
        } catch (error) {
                            ...
                            <table className="w-full text-left">
                                ...
                                <tbody className="divide-y divide-white/5">
                                    {loading ? (
                                        ...
                                        logs.map((log) => (
                                            <Fragment key={log.id}>
                                            <tr onClick={() => setExpandedLog(expandedLog === log.id ? null : log.id)} className={`hover:bg-white/5 transition-colors group cursor-pointer ${expandedLog === log.id ? 'bg-white/5' : ''}`}>
                                            ...
                                            {expandedLog === log.id && log.tools_used && log.tools_used.length > 0 && (
                                                <tr>
                                                    <td colSpan={5} className="px-6 py-4 bg-black/40 border-t border-white/5">
                                                        <div className="space-y-3">
                                                            <h4 className="text-xs font-bold uppercase tracking-widest text-purple-400 flex items-center gap-2">
                                                                <Code className="w-4 h-4" /> Herramientas Utilizadas
                                                            </h4>
                                                            {log.tools_used.map((tool: any, idx: number) => (
                                                                <div key={idx} className="bg-white/5 rounded-lg p-3 space-y-2 border border-white/10">
                                                                    <div className="text-[10px] font-bold text-blue-300 uppercase">→ {tool.name}</div>
                                                                    <div className="grid grid-cols-2 gap-4">
                                                                        <div>
                                                                            <span className="text-[9px] text-muted-foreground uppercase tracking-widest font-semibold flex mb-1">Argumentos (Input)</span>
                                                                            <pre className="text-[10px] font-mono text-emerald-300/80 whitespace-pre-wrap bg-black/30 p-2 rounded border border-white/5">
                                                                                {JSON.stringify(tool.input, null, 2)}
                                                                            </pre>
                                                                        </div>
                                                                        <div>
                                                                            <span className="text-[9px] text-muted-foreground uppercase tracking-widest font-semibold flex mb-1">Resultado (Output)</span>
                                                                            <pre className="text-[10px] font-mono text-foreground/70 whitespace-pre-wrap line-clamp-6 hover:line-clamp-none bg-black/30 p-2 rounded border border-white/5">
                ...
                <div className="space-y-6">
                    ...
                    <div className="space-y-4">
                        ...
                        <div className="glass-card p-5 flex flex-col">
                            <h3 className="text-sm font-semibold mb-2 text-muted-foreground uppercase tracking-widest flex items-center gap-2">
                                <Code className="w-4 h-4 text-emerald-400" />
                                Prompt Dinámico
                            </h3>
                            <div className="inline-flex items-center gap-1.5 px-2 py-1 bg-emerald-500/10 border border-emerald-500/20 text-emerald-400 text-[10px] rounded mb-3 w-fit">
                                <Clock className="w-3 h-3" />
                                Última act.: {promptData?.lastUpdated ? new Date(promptData.lastUpdated).toLocaleString('es-MX', { day: '2-digit', month: 'short', hour: '2-digit', minute:'2-digit' }) : 'Cargando...'}
                            </div>
                            <div className="bg-black/40 rounded-lg p-3 max-h-64 overflow-y-auto border border-white/5 scrollbar-hide flex-1">
                                <pre className="text-[10px] font-mono text-foreground/70 whitespace-pre-wrap leading-relaxed">
                                    {promptData?.prompt || 'Cargando contexto dinámico...'}
                                </pre>
                            </div>
                        </div>
