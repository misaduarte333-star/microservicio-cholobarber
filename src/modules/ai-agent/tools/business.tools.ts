...
export function makeConsultarServiciosTool(sucursalId: string) {
  return tool(
    async () => {
...
export function makeConsultarBarberosTool(sucursalId: string) {
  return tool(
    async () => {
      const { data, error } = await supabase
        .from('barberos')
...
export function makeConsultarSucursalTool(sucursalId: string) {
  return tool(
    async () => {
      const { data, error } = await supabase
        .from('sucursales')
...
export function makeConsultarBloqueosTool(sucursalId: string) {
  return tool(
    async () => {
      const { data, error } = await supabase
        .from('bloqueos')
...
export function makeConsultarTendenciasTool(sucursalId: string) {
  return tool(
    async () => {
      const { data, error } = await supabase
        .from('vista_tendencias_servicios')
        .select('*')
        .eq('sucursal_id', sucursalId);
...
export function makeEnviarFotosCortesTool(sucursalId: string, evolutionConfig: EvolutionConfig) {
  return tool(
    async ({ cliente_telefono, barbero_nombre, servicio_nombre }) => {
      ...
      for (const foto of data) {
        ...
        await EvolutionService.sendMedia(cliente_telefono, foto.url, `${servicio} por ${barbero} ✂️`, evolutionConfig);
      }
    ...
    },
    {
      ...
      description:
        'Envía fotos de cortes por WhatsApp al cliente. Úsala cuando el cliente pida ver estilos, cortes disponibles o tendencias. Si el cliente menciona un barbero o servicio por nombre, pásalo como texto — la tool resuelve el UUID internamente.',
      schema: z.object({
...
// ── Legacy named exports (backward compat for chat.controller / tests) ────────
// These use an empty sucursalId which returns data across all tenants — only
// safe for the /chat debug endpoint where multi-tenancy doesn't apply.
export const consultarServiciosTool  = makeConsultarServiciosTool('');
export const consultarBarberosTool   = makeConsultarBarberosTool('');
export const consultarSucursalTool   = makeConsultarSucursalTool('');
export const consultarBloqueosTool   = makeConsultarBloqueosTool('');
export const consultarTendenciasTool = makeConsultarTendenciasTool('');
export const enviarFotosCortesTool   = makeEnviarFotosCortesTool('', { instance: '', url: '', key: '' });
