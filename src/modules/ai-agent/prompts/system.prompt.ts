...
const PERSONALITY_DESCRIPTIONS: Record<string, string> = {
  'friendly':       'informal, cercano, usa emojis con moderación ✂️ 💈 😊',
  'professional':   'formal, puntual, sin emojis, respuestas concisas',
  'casual':         'muy relajado, usa jerga amigable, emojis frecuentes',
  'cholo-friendly': 'informal cholo, directo, con emojis ✂️ 💈, estilo barrial amigable',
};
...
Estilo: ${personalityDesc}

${greetingLine}

${ctx.customPrompt ? `INSTRUCCIONES ADICIONALES DE ${ctx.nombre.toUpperCase()}:\n${ctx.customPrompt}\n` : ''}
...
/** @deprecated Use buildSystemPrompt(ctx) instead. Kept for backward compatibility. */
export const SYSTEM_PROMPT = buildSystemPrompt({
  ...
  evolution: { instance: 'barberia', url: '', key: '' },
  llm: { provider: 'openai' },
});
